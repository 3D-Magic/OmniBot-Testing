...
class AlpacaConfig(BaseBroker):
    ...
    def get_bars(self, symbol: str, timeframe: str = '1Day', limit: int = 100):
        """Get historical price bars for technical analysis - with yfinance fallback"""
        if not self._connected:
            return None
        ...
        try:
            ...
            if bars and bars.df is not None and len(bars.df) > 0:
                return bars.df
            
            # Fallback to Yahoo Finance
            logger.info(f"[ALPACA BARS] {symbol}: Alpaca returned empty, trying yfinance...")
            return self._get_yfinance_bars(symbol, timeframe, limit)
            
        except Exception as e:
            logger.warning(f"[ALPACA BARS ERROR] {symbol}: {e}, trying yfinance...")
            return self._get_yfinance_bars(symbol, timeframe, limit)
    
    def _get_yfinance_bars(self, symbol: str, timeframe: str = '5Min', limit: int = 30):
        """Fallback to Yahoo Finance for free historical data"""
        try:
            from .yfinance_client import get_yf_client
            yf = get_yf_client()
            # Map Alpaca timeframe to yfinance interval
            interval_map = {'5Min': '5m', '1Min': '1m', '15Min': '15m', '1Hour': '1h', '1Day': '1d'}
            interval = interval_map.get(timeframe, '5m')
            return yf.get_bars(symbol, interval=interval, limit=limit)
        except Exception as e:
            logger.error(f"[YFINANCE FALLBACK ERROR] {symbol}: {e}")
            return None
